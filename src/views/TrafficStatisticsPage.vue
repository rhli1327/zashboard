...
const rebuildQueryWindow = () => {
  ...
  const localDayStart = new Date(to.getFullYear(), to.getMonth(), to.getDate())
  if (selectedRange.value === 'today') {
    frozenWindow.value = {
      from: localDayStart.toISOString(),
      to: (to > localDayStart ? to : new Date()).toISOString(),
    }
    return
  }
  if (selectedRange.value === 'yesterday') {
    frozenWindow.value = {
      from: new Date(to.getFullYear(), to.getMonth(), to.getDate() - 1).toISOString(),
      to: localDayStart.toISOString(),
    }
    return
  }
  if (selectedRange.value === 'week') {
    const daysSinceMonday = (localDayStart.getDay() + 6) % 7
    const weekStart = new Date(
      localDayStart.getFullYear(),
      localDayStart.getMonth(),
      localDayStart.getDate() - daysSinceMonday,
    )
    frozenWindow.value = {
      from: weekStart.toISOString(),
      to: (to > weekStart ? to : new Date()).toISOString(),
    }
    return
  }
  if (selectedRange.value === 'month') {
    ...
    frozenWindow.value = {
      ...
      to: (to > monthStart ? to : new Date()).toISOString(),
    }
    return
  }
  if (selectedRange.value === 'last_month') {
    frozenWindow.value = {
      from: new Date(to.getFullYear(), to.getMonth() - 1, 1).toISOString(),
      to: new Date(to.getFullYear(), to.getMonth(), 1).toISOString(),
    }
    return
  }
