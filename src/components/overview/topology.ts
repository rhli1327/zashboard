...
export const buildTopologyData = (
  ...
  const nodeSourceIPGeoIPs = new Map<string, string>()
  const linkCounts = new Map<string, number>()
  let nextNodeId = 0

  const addNode = (name: string, layer: number, type: string, sourceIPGeoIP?: string) => {
    ...
    if (sourceIPGeoIP) {
      nodeSourceIPGeoIPs.set(key, sourceIPGeoIP)
    }
  ...
  for (const connection of connections) {
    ...
    const sourceNode = addNode(
      connection.source,
      0,
      labels.sourceIPAddress,
      connection.sourceIPGeoIP,
    )
  ...
  const initialNodes = Array.from(nodeIds, ([key, id]): TopologyNode => {
    ...
    return {
      ...
      sourceIPGeoIP: nodeSourceIPGeoIPs.get(key),
      layer,
      itemStyle: {
