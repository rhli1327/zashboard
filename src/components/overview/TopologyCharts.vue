...
const topologyData = computed(() =>
  buildTopologyData(
    activeConnections.value.map((connection) => {
      const sourceIPAddress = getConnectionSourceIP(connection)

      return {
        source: getIPLabelFromMap(sourceIPAddress),
        sourceIPGeoIP: formatGeoIPInfo(getGeoIPInfoSync(sourceIPAddress, true)),
...
const options = computed<EChartOption>(() => ({
  ...
  tooltip: {
    ...
    formatter: (params: unknown) => {
      ...
      if (dataType === 'node') {
        const sourceIPGeoIP = data.sourceIPGeoIP

        return `${escapeChartHtml(data.name)}<br/>${escapeChartHtml(t('nodeType'))}: ${escapeChartHtml(
          data.nodeType || t('unknown'),
        )}${
          sourceIPGeoIP
            ? `<br/>${escapeChartHtml(t('sourceIPGeoIP'))}: ${escapeChartHtml(sourceIPGeoIP)}`
            : ''
        }`
      }
      ...
      if (dataType === 'edge') {
        ...
        if (sourceNode && targetNode) {
          return `${escapeChartHtml(sourceNode.name)} → ${escapeChartHtml(targetNode.name)}${
            sourceNode.sourceIPGeoIP
              ? `<br/>${escapeChartHtml(t('sourceIPGeoIP'))}: ${escapeChartHtml(
                  sourceNode.sourceIPGeoIP,
                )}`
              : ''
  ...
  series: [
    {
      ...
      label: {
        ...
        formatter: (params: { name: string; data: { sourceIPGeoIP?: string } }) => {
          ...
          return params.data.sourceIPGeoIP
            ? `${name}\n${truncate(params.data.sourceIPGeoIP)}`
            : name
        },
      },
      nodeGap: 4,
